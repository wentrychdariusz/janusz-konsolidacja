      ...
      <div className="relative z-10 px-4 md:px-8 lg:px-12 xl:px-16 max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-3xl shadow-2xl overflow-hidden relative">
            ...
            <div className="px-8 pb-12">
              <div className="grid grid-cols-3 gap-8">
                {getCurrentDesktopTestimonials().map((testimonial, index) => <div key={`${currentDesktopSet}-${index}`} className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-300">
                    ...
                    <div className="relative w-full h-64 rounded-xl overflow-hidden mb-6 bg-gray-100 group">
                      ...
                      {index === 0 && <button onClick={prevDesktopSet} className="absolute left-2 top-1/2 transform -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all duration-300 opacity-0 group-hover:opacity-100">
                          <ChevronLeft className="w-5 h-5 text-gray-700" />
                        </button>}
                      
                      {index === getCurrentDesktopTestimonials().length - 1 && <button onClick={nextDesktopSet} className="absolute right-2 top-1/2 transform -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all duration-300 opacity-0 group-hover:opacity-100">
            ...
            }).map((_, index) => <button key={index} onClick={() => setCurrentDesktopSet(index)} className={`w-3 h-3 rounded-full mx-1 transition-colors duration-300 ${index === currentDesktopSet ? 'bg-blue-600' : 'bg-gray-300 hover:bg-gray-400'}`} />)}
            </div>
          </div>
        </div>
        ...
        <div className="block lg:hidden mb-8">
          <div className="relative">
            ...
            <div className="bg-slate-800 rounded-3xl overflow-hidden shadow-2xl mx-4 select-none" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
              {/* Large profile image with navigation overlay */}
              <div className="relative w-full h-80 bg-gray-100 rounded-t-3xl overflow-hidden">
                ...
                <button onClick={prevTestimonial} className="absolute left-4 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all duration-300">
                  <ChevronLeft className="w-6 h-6 text-gray-700" />
                </button>
                
                <button onClick={nextTestimonial} className="absolute right-4 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-all duration-300">
                  <ChevronRight className="w-6 h-6 text-gray-700" />
                </button>

                {/* Swipe hint indicator */}
                {isAutoPlaying && <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black/50 text-white text-xs px-3 py-1 rounded-full animate-pulse">
                    Przesuń palcem →
                  </div>}
              </div>
            ...
            }} className={`w-3 h-3 rounded-full transition-colors duration-300 ${index === currentTestimonial ? 'bg-white' : 'bg-white/40 hover:bg-white/60'}`} />)}
            </div>

            {/* Autoplay control */}
            <div className="flex justify-center mt-4">
              <button onClick={() => setIsAutoPlaying(!isAutoPlaying)} className="text-white/60 hover:text-white text-sm transition-colors duration-300">
